...
*****************************************************************************************/
#define USER_DEVICE_NAME    "IFM RCU Unit"
...
static const struct gapm_configuration user_gapm_conf = {
    ...
    .max_mtu = MAX_MTU_SIZE, // Maximal MTU
    ...
    ***************************************************/
    ...
    .max_txoctets = CFG_MAX_TX_PACKET_LENGTH,             // Maximal Tx octets
    .max_txtime = (CFG_MAX_TX_PACKET_LENGTH+11+3)*8,      // Maximal Tx time
};
...
static const struct connection_param_configuration user_connection_param_conf = {
    ...
    .latency = PARAM_UPDATE_LATENCY,                  // Latency measured in connection events
...
/******************************************************************************
 * \brief Define USE_ONE_BAS_INSTANCE to use only one BASS instance
*******************************************************************************/
#define USE_ONE_BAS_INSTANCE


/******************************************************************************
 * \brief Define USE_ONE_HIDS_INSTANCE to use only one HOGPD instance
*******************************************************************************/
...
#ifndef HAS_MOTION
    /***************************************************************************
     * \brief Define CONFIGURE_MOTION_SENSOR_PINS when motion sensor is 
     * connected to DA14585 but not used, in order to configure the sensor
     * interface pins to their deasserted state to avoid current leakage.
    ***************************************************************************/
    #define CONFIGURE_MOTION_SENSOR_PINS
#endif
...
#undef HAS_IR
    ...
    static const uint16_t kbd_ir_keymap[][3] =
    {
        /* 
          0          1         2      
          -------------------------------------
        ...
        { 0x0007,    0x0008,   0x0009  },  // 2
        { K_CODE,    0x0000,   K_CODE  }   // 3
    };
#endif
...
*******************************************************************************/
#ifndef NORMALLY_CONNECTABLE
    #define HAS_PWR_MGR
#endif    


/*************************************************************************************
 * \brief Define HAS_WKUP to enable wakeup controller sharing between various modules
*************************************************************************************/
#if defined(HAS_KBD) || defined(HAS_TOUCHPAD_TRACKPAD) || defined(HAS_TOUCHPAD_SLIDER) || defined(HAS_GPIO_KEYS)
    #define HAS_WKUP
#endif    
...
/**
 ******************************************************************************
 * \brief Define AUDIO_TEST_MODE to enable audio test mode. Test mode is
 * activated by a key combination. Audio streaming is activated periodically 
 * for a few seconds 
 ******************************************************************************
 */
#if defined(HAS_AUDIO) && defined(HAS_KBD)
    #undef AUDIO_TEST_MODE
#endif

/**
 ******************************************************************************
 * \brief Define ENABLE_OTA_DEBUG to enable over the air debugging using 
 * Dialog Audio BLE service.
 ******************************************************************************
 */
//#define ENABLE_OTA_DEBUG
