...
static audio_sample_t *get_next_audio_buffer(uint16_t length)
{
    ...
    if (port_audio_env.audioSlotWrNr == AUDIO_BUFFER_NR_SLOTS) {
        port_audio_env.audioSlotWrNr = 0;
        //ASSERT_WARNING(port_audio_env.audioSlots[port_audio_env.audioSlotWrNr].hasData == false)
    }
...
static void commit_audio_buffer(void)
{
    ...
    if(port_audio_env.write_index == AUDIO_BUFFER_SIZE) {
...
const int32_t sine_a[72]={          0,    93582720,   186453120,   277904640,   367240960,   453782528,   536870528,   615872512,   690187392,
