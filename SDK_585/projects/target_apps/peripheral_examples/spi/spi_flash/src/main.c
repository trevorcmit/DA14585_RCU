...
static void spi_flash_peripheral_init()
{
    ...
    detected_spi_flash_device_index = spi_flash_enable(SPI_GPIO_PORT, SPI_CS_PIN);
...
static void print_MX25R_power_mode()
{
    int32_t power_mode;
    // MX25Rxx35Lx default is LPM or Low Power Mode
    // Get the power mode
    power_mode = spi_flash_MX25R_get_power_mode();
    printf_string("\n\r\n\rMX25R Power Mode: ");
    if ((power_mode & MX25R_HPM)== MX25R_HPM)
        printf_string(" High Performance Mode ");
    else
        printf_string(" Low Power Mode ");
}
...
void spi_test(void)
{
    ...
    switch (detected_spi_flash_device_index)
    {
        ...
        case SPI_FLASH_DEVICE_INDEX_MX25R1035F:
            printf_string("\n\rYou are using MX25R1035F 1-Mbit SPI flash device.");
        break;

        case SPI_FLASH_DEVICE_INDEX_MX25R2035F:
    ...
    if ((detected_spi_flash_device_index == SPI_FLASH_DEVICE_INDEX_MX25R1035F) ||
        (detected_spi_flash_device_index == SPI_FLASH_DEVICE_INDEX_MX25R2035F))
    {
        //---------------- MX25R Low Power Mode Tests -------------------------------

        spi_flash_MX25R_set_power_mode(MX25R_LPM);
        ...
        spi_flash_MX25R_set_power_mode(MX25R_HPM);
