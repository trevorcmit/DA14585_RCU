...
static void usage(const char* my_name)
{
	fprintf(stderr,
		...
		"  or\n"
		"\t#define SW_VERSION \"v_1.0\"\n"
		"\t#define SW_VERSION_DATE \"2014-06-30 17:56 \"\n"
		"\n"
		"\n"
		"Usage case #2:\n"
...
static int parse_version_file(FILE* f, struct image_header* hdr)
{
	...
	do {
		...
		if (patterns_left & IMG_DATE) {
			...
			if(from == NULL) 
				from = strstr(line, "SW_VERSION_DATE ");
			if (from) {
				s = find_cstring(from);
				if (s)
