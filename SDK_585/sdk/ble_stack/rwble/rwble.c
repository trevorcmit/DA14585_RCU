...
bool                skip_slave_latency_flag         __attribute__((section("retention_mem_area0"), zero_init));
#if (BLE_METRICS)
...
uint8_t packets_per_event      __attribute__((section("retention_mem_area0"), zero_init));
uint8_t good_packets_per_event __attribute__((section("retention_mem_area0"), zero_init));
uint8_t max_packets_per_event  __attribute__((section("retention_mem_area0"), zero_init));
uint32_t ble_throughput_rx_err __attribute__((section("retention_mem_area0"), zero_init));
uint32_t ble_throughput_rx_pkt __attribute__((section("retention_mem_area0"), zero_init));

static void update_ble_throughput_metrics(uint8_t pkts, bool evt_end)
{
    uint8_t rx_cnt = pkts-packets_per_event;
    uint8_t rx_hdl = co_buf_rx_current_get();
    uint8_t status;
    ...
    while(rx_cnt--)
    {
        ...
        if(packets_per_event > max_packets_per_event && (status & BLE_SYNC_ERR_BIT)) {
            // We do not count the packet if we get SYNC error in the last packet of the connection event
        }
        else {        
            if (status & (BLE_MIC_ERR_BIT | BLE_CRC_ERR_BIT | BLE_LEN_ERR_BIT | BLE_TYPE_ERR_BIT | BLE_SYNC_ERR_BIT | BLE_SN_ERR_BIT | BLE_NESN_ERR_BIT)) {
                    ble_throughput_rx_err += max_packets_per_event - packets_per_event;
            }
            else {
                good_packets_per_event++;
                ble_throughput_rx_pkt++;
            }
        }
    ...
    if(good_packets_per_event > max_packets_per_event) {
        max_packets_per_event = good_packets_per_event;
    }
    
    if(evt_end == true) {
        packets_per_event = 0;
        good_packets_per_event = 0;
    }
}
#endif // BLE_THROUGHPUT_METRICS
...
__weak void user_cscnt_isr(void) {}
...
static void skip_slave_latency(void)
{

#if (BLE_PERIPHERAL)
    if (skip_slave_latency_flag == true) {
    // Pop the element in the programmed environment queue
        ...
        if ((evt->instant_action == LLD_EVT_NO_ACTION)) {
...
#endif // (BLE_METRICS)
#ifdef BLE_THROUGHPUT_METRICS
    update_ble_throughput_metrics(LLD_RX_IRQ_THRES, false);
#endif    
    lld_evt_rx_isr();
}

__weak void user_event_isr(void) {}
...
{
#if (BLE_METRICS) || defined(BLE_THROUGHPUT_METRICS)
    {
...
#endif        
#ifdef BLE_THROUGHPUT_METRICS        
        update_ble_throughput_metrics(rx_cnt, true);
#endif        
    }
#endif // (BLE_METRICS)
