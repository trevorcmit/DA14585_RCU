...
{
    struct custs1_env_tag *custs1_env = PRF_ENV_GET(CUSTS1, custs1);
    ASSERT_ERR(custs1_env->operation != NULL);
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    custs1_get_att_idx(ccc_hdl, &ccc_idx);

    custs1_att_get_value(ccc_idx, &ccc_length, &ccc_val);
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    // Find the handle of the Characteristic Client Configuration
    cfg_hdl = get_cfg_handle(custs1_env->shdl + param->handle);
    ASSERT_ERR(cfg_hdl);
