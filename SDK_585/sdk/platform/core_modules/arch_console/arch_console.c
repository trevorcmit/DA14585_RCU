...
static bool block_sleep __attribute__((section("retention_mem_area0"), zero_init));
static bool arch_force_active __attribute__((section("retention_mem_area0"), zero_init));
...
static void uart_callback(uint8_t res)
{
    ...
    if (printf_msg_list) {
        ...
        uart2_write_from_isr((uint8_t *)p, arch_strlen(p), uart_callback);
#else
...
void arch_printf_process(void)
{
    if (defer_sending) {
        ...
        if(block_sleep == true && printf_msg_list == NULL) {
#ifdef CFG_PRINTF_UART2            
            if(uart2_thre_getf() != 0 && uart2_temt_getf() != 0) {
#else
            if(uart_thre_getf() != 0 && uart_temt_getf() != 0) {
#endif
                // If all characters in the Tx FIFO and shift register
                // have been sent then restore sleep mode
                arch_restore_sleep_mode();
            
                block_sleep = false;
                arch_force_active = false;
            }
            else {
                // Otherwise stay active to send the remaining characters
                arch_force_active = true;
            }
        }
    }
}
