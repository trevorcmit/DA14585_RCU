...
int main(void) __attribute__((noreturn));
    ...
    #define SMOOTHING_FACTOR 64
    uint8_t cpu_load_index                     __attribute__((section("retention_mem_area0"), zero_init));
    uint8_t cpu_load                           __attribute__((section("retention_mem_area0"), zero_init));
    uint32_t cpu_load_active[SMOOTHING_FACTOR] __attribute__((section("retention_mem_area0"), zero_init));
    uint32_t cpu_load_idle[SMOOTHING_FACTOR]   __attribute__((section("retention_mem_area0"), zero_init));
    uint32_t cpu_load_prev_timestamp           __attribute__((section("retention_mem_area0"), zero_init));
    
    uint32_t get_timestamp(void)
    {
        uint32_t ea_time_get_halfslot_rounded(void);   
        return ea_time_get_halfslot_rounded();
//        uint32_t value = (GetWord16(CLK_REF_VAL_H_REG) << 16) + GetWord16(CLK_REF_VAL_L_REG);
//        SetWord16(CLK_REF_SEL_REG, 0); // select xtal32kHz oscillator, start!
//        SetWord16(CLK_REF_CNT_REG, 0x0);
//        while(GetBits16(CLK_REF_SEL_REG, REF_CAL_START) == 1);
//        SetWord16(CLK_REF_CNT_REG, 0xFFFF);
//        SetWord16(CLK_REF_SEL_REG, 6); 
...
int main(void)
{
    ...
    while(1)
    {
        ...
        if (((!BLE_APP_PRESENT) && (check_gtl_state())) || (BLE_APP_PRESENT))
        {
...
#if defined(MEASURE_CPU_LOAD) && DEVELOPMENT_DEBUG
            uint32_t current;
            // get the timestamp before powering-off the BLE
            ...
            sleep_mode_t temp_sleep_mode = sleep_mode;
            app_sleep_prepare_proc(&temp_sleep_mode);
            if(temp_sleep_mode == mode_active) {
            ...
            if ((sleep_mode == mode_ext_sleep) || (sleep_mode == mode_ext_sleep_otp_copy))
            {
                ...
                GPIO_ConfigurePin(IDLE_INDICATION_PORT, IDLE_INDICATION_PIN, OUTPUT, PID_GPIO, true );
#endif        
#if defined(MEASURE_CPU_LOAD) && DEVELOPMENT_DEBUG
                cpu_load_index = 0;
//                cpu_load_active[cpu_load_index] = current - cpu_load_prev_timestamp;
//                if(cpu_load_prev_timestamp != 0 && cpu_load_active[cpu_load_index] > 0x150) {
//                    ASSERT_ERROR(0);
//                }
...
#if defined(INDICATE_IDLE_STATE) && DEVELOPMENT_DEBUG
            GPIO_ConfigurePin(IDLE_INDICATION_PORT, IDLE_INDICATION_PIN, OUTPUT, PID_GPIO, false );
#endif                           
            // restore interrupts
            GLOBAL_INT_START();
        }
